...
BETWEEN strftime('%s', starting_time) AND strftime('%s', ending_time);

$r2=$db->query("update tasks set username=:ne",array("ne"=>$randomName));

$r2=$db->query("update tasks set username=(select username from user order by random() limit 1)");
